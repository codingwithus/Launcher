...
class LiveFolderAdapter extends CursorAdapter {
	...
	public void bindView(View view, Context context, Cursor cursor) {
		...
		if (!mIsList) {
			holder.name.setCompoundDrawablesWithIntrinsicBounds(null, icon,
					null, null);
		} else {
			...
			if (holder.descriptionIndex != -1) {
				final String description = cursor
						.getString(holder.descriptionIndex);
				if (description != null) {
		...
		if (holder.intentIndex != -1) {
			try {
				holder.intent = Intent.parseUri(
						cursor.getString(holder.intentIndex), 0);
	...
	private Drawable loadIcon(Context context, Cursor cursor, ViewHolder holder) {
		...
		if (data != null) {
			final SoftReference<Drawable> reference = mCustomIcons
					.get(holder.id);
			if (reference != null) {
				icon = reference.get();
			}
			...
			if (icon == null) {
				 ...
				 * modify by author
				 */
				// final Bitmap resampled = Utilities.resampleIconBitmap(bitmap,
				// mContext);
				final Bitmap resampled = Utilities.resampleIconBitmap(bitmap,
						(Context) mLauncher);

				if (bitmap != resampled) {
					// If we got back a different object, we don't need the old
					// one any more.
					bitmap.recycle();
				}
		...
		} else if (holder.iconResourceIndex != -1
				&& holder.iconPackageIndex != -1) {
			...
			if (icon == null) {
				try {
					...
					Resources resources = packageManager
							.getResourcesForApplication(cursor
									.getString(holder.iconPackageIndex));
					final int id = resources
							.getIdentifier(resource, null, null);
					/**
					 * mContext������ı������ڹ���ʱͨ��super(launcher, cursor,
					 * true)��
					 * launcher��ֵ��mContext�����mContext��mLauncher����ͬһ������
					 * modify by author
					 */
					...
					// mContext));
					icon = new FastBitmapDrawable(Utilities.createIconBitmap(
							resources.getDrawable(id), mLauncher));
