...
public class ClippedImageView extends ImageView {
	...
	public ClippedImageView(Context context, AttributeSet attrs, int defStyle) {
		...
		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.ClippedImageView, defStyle, 0);

		mZone = a.getDimensionPixelSize(
				R.styleable.ClippedImageView_ignoreZone, 0);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		return !(zone != 0 && (zone > 0 && event.getX() >= zone) || (zone < 0 && event
				.getX() < getWidth() + zone)) && super.onTouchEvent(event);
